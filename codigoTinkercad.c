...
Servo myservo;  //Crea el objeto servo para controlar el servomotor
...
void setup() {
  ...
  //Setea los pines de control del sensor ultrasonico
  ...
  myservo.attach(9);  //Une el pin 9 al servo
...
char direccionOptima(){
  ...
  if((distanciaMedida < distanciaOptima) && (distanciaMedida > 10)){
...
int distancia(){
  ...
  delayMicroseconds(5);
  // Dispara el sosnsor, seteando el pin trigger en alto por 10ms
  digitalWrite(trigPin, HIGH);
  delayMicroseconds(10);
  digitalWrite(trigPin, LOW);
  //Lee el pin echo, la funcion pulseIn() retorna la duracion del pulso en microsegundos
